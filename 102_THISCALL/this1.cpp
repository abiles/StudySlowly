...
class Point
{
	int x = 0, y = 0;
public:
	void set(int a, int b) // void set(Point* const this, int a, int b) < - this가 포함됨을 확인 *1번
	{
		x = a;	// this->x = a;
		y = b;	// this->y = b;
	}
	static void foo(int a) // void foo( int a) <- static은 객체없이 호출할 수 있다. this를 포함하지 않는다 * 2번
	{
